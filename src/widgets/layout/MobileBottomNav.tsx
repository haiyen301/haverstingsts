...
export function MobileBottomNav({ showInventoryImport, user = null }: MobileBottomNavProps) {
  ...
  const moreItems: NavEntry[] = [
    ...
    { path: "/inventory", labelKey: "inventory", icon: BarChart3, module: "inventory" },
    { path: "/planning", labelKey: "planning", icon: Calendar },
    { path: "/forecasting", labelKey: "forecasting", icon: BarChart3, module: "forecasting" },
    ...(showInventoryImport
      ? ([
          {
  ...
  const visibleMoreItems = moreItems.filter(
    (item) => !item.module || canAccessModule(user, item.module, "show"),
  );

  const isActive = (path: string) => {
    if (path === "/projects") return pathname.startsWith("/projects");
    if (path === "/harvest") return pathname.startsWith("/harvest") && !pathname.startsWith("/harvest/schedule");
    if (path === "/forecasting") return pathname.startsWith("/forecasting");
    if (path === "/inventory") return pathname.startsWith("/inventory");
    if (path === "/inventory-import") return pathname.startsWith("/inventory-import");
    return pathname === path;
  };

  const moreActive = visibleMoreItems.some((item) => isActive(item.path));
