...
export async function POST(req: Request) {
  ...
  try {
    const result = await writeProjectListToUserGoogleSheet({
      ...
      config,
      payload: body,
      stsAuthorization: auth,
    });
    return NextResponse.json({
      ok: true,
      message: "Exported to your Google Drive.",
...
/** OAuth + connection status for export dialog. */
export async function GET(req: Request) {
  ...
  return NextResponse.json({
    ...
    authorizePath: "/api/projects/export/google-sheet/oauth/authorize",
    callbackPath: "/api/projects/export/google-sheet/oauth/callback",
  });
}
